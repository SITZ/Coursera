...
__global__ void matrixMultiplyShared(float * A, float * B, float * C, int numARows, int numAColumns, int numBRows, int numBColumns, int numCRows, int numCColumns) {
	...
	for (int tileNumber = 0; tileNumber <= numAColumns / TILE_WIDTH; tileNumber++) {
		...
		int actualWidth = numAColumns - tileStart;
      	if (actualWidth > TILE_WIDTH) {
        	actualWidth = TILE_WIDTH;
      	}
		
      	if (tx < actualWidth && Row < numARows) {
    		sharedA[ty][tx] = A[Row * numAColumns + tileStart + tx];
        }
      	if (ty < actualWidth && Col < numBColumns) {
    		sharedB[ty][tx] = B[(tileStart + ty) * numBColumns + Col];
        }
    
		__syncthreads();
		for (int k = 0; k < actualWidth; k++) {
			Cval += sharedA[ty][k] * sharedB[k][tx];
		}
	}
...
int main(int argc, char ** argv) {
    ...
    dim3 dimGrid(ceil(numCRows / (float) TILE_WIDTH), ceil(numCColumns / (float) TILE_WIDTH), 1);
